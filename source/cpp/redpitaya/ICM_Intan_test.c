...
int main(int argc, char **argv)
{
    ...
    printf("Initializing sensor, then sampling every 1 ms (%u clock ticks).\n",
           SENSOR_TEST_1MS_TICKS);
    printf("ICM+Intan DMA packet: %u bytes (%u 32-bit words).\n",
           SENSOR_TEST_ICM_INTAN_PACKET_BYTES,
           SENSOR_TEST_ICM_INTAN_PACKET_WORDS);
    ...
    if (use_reserved) {
        if (sensor_test_prepare_dma_reserved_sized(
                ...
                SENSOR_TEST_ICM_INTAN_PACKET_BYTES) != 0)
            goto cleanup;
